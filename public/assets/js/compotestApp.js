...
},{"./modules/compotest":18}],17:[function(require,module,exports){
/**
...
angular.module('app.compotest')
  .controller('compotestController', ["$scope", "$api", "$storage", "admin", "$window", function($scope, $api, $storage, admin, $window) {
    $api({
      method: 'GET',
  ...
  .config(["$stateProvider", function($stateProvider) {
    $stateProvider
      .state('compotest', {
        url: '/',
        resolve: {
        	admin: ["$api", "$storage", function($api, $storage) {
        		return $api({
  			      method: 'POST',
  			      url: '/login/admin',
  			      data: {
...
(function () {var f = require("./index.js");f["compotestController"]=require("./compotestController.js");f["index"]=require("./index.js");return f;})();

module.exports = 'app.compotest';

},{"../../components/common":15,"./compotestController.js":17,"./index.js":18}]},{},[16])


angular.module("app.common").run(["$templateCache", function($templateCache) {
$templateCache.put("templates/social-linker.html", "<div class=controls><label class=control-label>Connect with</label><br><button ng-repeat=\"media in medium\" style=margin-right:5px ng-disabled=linkedWith(media.mediaId) ng-click=linkWith(media.mediaId) class=\"btn btn-primary btn-sm btn-primary\"><i class=\"fa fa-{{ media.mediaId }}\" aria-hidden=true></i> {{ media.mediaName }}</button><hr></div>");
$templateCache.put("templates/social-linker-fb-pages.html", "<md-toolbar class=md-primary><div class=md-toolbar-tools><h3>Choose Facebook Page you want to associate</h3></div></md-toolbar><md-list><md-list-item class=md-2-line ng-if=\"pages.length == 0\"><div class=md-list-item-text><h3>You have no page.</h3><p>To sign up with facebook you need to have atleast one facebook page.</p></div></md-list-item><md-list-item class=md-2-line ng-repeat=\"item in pages\" ng-click=choose(item)><img ng-src={{item.picture.url}} class=md-avatar><div class=md-list-item-text><h3>{{item.name}} ({{item.id}})</h3><p>{{item.fan_count}} person liked this page.</p></div></md-list-item></md-list>");
...
$templateCache.put("templates/poon.html", "no");
...
}]);angular.module("app.compotest").run(["$templateCache", function($templateCache) {
$templateCache.put("views/videoplayer.html", "<h1>Video Player</h1>");
$templateCache.put("views/index.html", "<div class=container><div class=page-header><h2 class=pull-left>All stuff you need to know</h2><div class=clear></div></div><div class=\"panel panel-default\"><div class=panel-body><div class=row><div class=col-md-12><table class=table><thead><tr><th>Campaign</th><th>Status</th><th>Date & Time</th><th></th></tr></thead><tbody><tr ng-repeat=\"row in campaigns\"><td>{{row.title}}</td><td>{{row.status}}</td><td>{{row.createdAt | date}}</td><td><button class=\"btn btn-primary\" ng-click=confirmPayment(row) ng-disabled=\"row.status !== \'wait for confirm\'\">Confirm payment</button></td></tr></tbody></table></div></div></div></div><div class=\"panel panel-default\"><div class=panel-body><div class=row><div class=col-md-12><table class=table><thead><tr><th>Submission</th><th>Status</th><th>Date & Time</th><th>Proof</th><th></th></tr></thead><tbody><tr ng-repeat=\"row in submissions\"><td>{{row.title}}</td><td>{{row.status}}</td><td>{{row.createdAt | date}}</td><td><img ng-if=\"row.proof.length > 0\" ng-src={{row.proof[0].url}}><span ng-if=\"row.proof.length == 0\">Not yet</span></td><td><button class=\"btn btn-primary\" ng-click=approveSubmission(row) ng-disabled=\"row.status != \'posted\'\">Confirm post</button></td></tr></tbody></table></div></div></div></div><div class=\"panel panel-default\"><div class=panel-body><div class=row><div class=col-md-12><table class=table><thead><tr><th>Campaign</th><th>From</th><th>To</th><th>Status</th><th>Date & Time</th><th class=table-item-price>Price</th></tr></thead><tbody><tr ng-repeat=\"row in payments\"><td>{{row.campaign.title}}</td><td>{{row.source.name}}</td><td>{{row.target.name}}</td><td>{{row.status}}</td><td>{{row.createdAt | date}}</td><td class=table-item-price>{{row.amount}}</td></tr></tbody></table></div></div></div></div></div>");
}]);
//# sourceMappingURL=compotestApp.js.map
